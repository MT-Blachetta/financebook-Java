...
public class FeeEngineService {
     ...
     * Mirrors Python's recompute_fee_record() function.
     * 
     * @param paymentItem Payment item
     * @param newAmount New raw amount (before fee)
     * @param userId User ID
     * @return New fee amount
     */
    @Transactional
    public BigDecimal recomputeFeeRecord(PaymentItem paymentItem, BigDecimal newAmount, Long userId) {
        TransactionFeeRecord existingRecord = feeRecordRepository.findByPaymentItemId(paymentItem.getId())
                .orElse(null);
        
        // Compute new fee based on the new raw amount
        BigDecimal newFee = computeFee(newAmount, userId);
        BigDecimal adjusted = (newFee.compareTo(BigDecimal.ZERO) > 0) 
                ? applyFeeToAmount(newAmount, newFee) 
                : newAmount;
        
        if (existingRecord != null) {
            if (newFee.compareTo(BigDecimal.ZERO) > 0) {
                existingRecord.setFeeAmount(newFee);
                existingRecord.setOriginalAmount(newAmount);
                feeRecordRepository.save(existingRecord);
            } else {
                feeRecordRepository.delete(existingRecord);
            }
        } else if (newFee.compareTo(BigDecimal.ZERO) > 0) {
            TransactionFeeRecord record = TransactionFeeRecord.builder()
                    .paymentItem(paymentItem)
                    .user(paymentItem.getUser())
                    .feeAmount(newFee)
